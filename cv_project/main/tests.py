...
class SkillModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.skill = Skill.objects.create(
            cv=self.cv, name="Python", proficiency="expert"
        )
...
class CVPDFDownloadTest(TestCase):
    ...
    def test_cv_pdf_download_status_code(self):
        ...
        response = self.client.get(
            reverse("cv_pdf_download", kwargs={"pk": self.cv.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_cv_pdf_download_with_special_characters(self):
        ...
        response = self.client.get(
            reverse("cv_pdf_download", kwargs={"pk": cv_special.pk})
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "application/pdf")
...
class RequestLogModelTest(TestCase):
    ...
    def test_full_url_property(self):
        ...
        log_no_query = RequestLog.objects.create(
            method="GET", path="/simple/", query_string="", remote_ip="127.0.0.1"
        )
    ...
    def test_response_time_seconds_property(self):
        ...
        log_no_time = RequestLog.objects.create(
            method="GET", path="/test/", remote_ip="127.0.0.1"
        )
    ...
    def test_get_recent_logs_class_method(self):
        ...
        for i in range(15):
            RequestLog.objects.create(
                method="GET", path=f"/test{i}/", remote_ip="127.0.0.1"
            )
...
class RequestLoggingMiddlewareTest(TestCase):
    ...
    def setUp(self):
        ...
        self.cv = CV.objects.create(
            firstname="Test", lastname="User", email="test@example.com", bio="Test bio"
        )
    ...
    def test_is_valid_ip_method(self):
        ...
        # Invalid IPs
        invalid_ips = ["256.1.1.1", "192.168", "not.an.ip", "192.168.1.1.1", "", None]
    ...
    def test_middleware_exclusion_logic(self):
        ...
        for path, should_log in test_cases:
            ...
            self.assertEqual(
                result,
                should_log,
                f"Path {path} should {'be logged' if should_log else 'be excluded'}",
            )
...
class ContextProcessorTest(TestCase):
    ...
    def test_context_processors_in_template(self):
        ...
        template = Template(
            "{{ settings.DEBUG }} {{ app_info.app_name }} {{ request_info.method }}"
        )
            ...
            for pattern in sensitive_patterns:
                if pattern in setting_name.upper():
                    ...
                    safe_settings = [
                        ...
                        "MIDDLEWARE_COUNT",
                        "IS_DEBUG_MODE",
                        "AUTH_PASSWORD_VALIDATORS",  # This might be present
                    ]
                    self.assertIn(
                        setting_name,
                        safe_settings,
