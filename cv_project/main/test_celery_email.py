    ...
    def test_pdf_generation_with_long_data(self):
        ...
        for i in range(10):
            Skill.objects.create(
                cv=self.cv, name=f"Skill {i}", proficiency="intermediate"
            )
        ...
        self.assertGreater(
            len(pdf_data), 3000
        )  # Should be larger PDF (reduced expectation)
        buffer.close()
...
class CeleryTasksTest(TestCase):
    ...
    @patch(
        "main.views.generate_cv_pdf_buffer"
    )  # Fixed: Import from main.views, not main.tasks
    ...
    def test_send_cv_pdf_email_invalid_cv(self):
        ...
        result = send_cv_pdf_email(
            cv_id=9999, recipient_email="test@example.com"  # Non-existent CV
        )
...
class EmailViewsTest(TestCase):
    ...
    @patch(
        "main.tasks.send_cv_pdf_email"
    )  # Fixed: Import from main.tasks, not main.views
    def test_email_cv_view_success(self, mock_task):
        ...
        data = {"email": "test@example.com", "sender_name": "Test User"}
    ...
    def test_check_email_task_status_success(self, mock_async_result):
        ...
        response = self.client.get(
            reverse("check_task_status", kwargs={"task_id": "test-task-id"})
        )
...
class EmailIntegrationTest(TestCase):
    ...
    def test_email_content_structure(self, mock_email_class):
        ...
        with patch(
            "main.views.generate_cv_pdf_buffer"
        ) as mock_pdf:  # Fixed: Import from main.views
            mock_buffer = MagicMock()
            mock_buffer.getvalue.return_value = b"fake_pdf_data"
...
class TaskConfigurationTest(TestCase):
    ...
    def test_task_routing_configuration(self):
        ...
        self.assertEqual(
            task_routes["cleanup_old_request_logs"]["queue"], "maintenance"
        )
