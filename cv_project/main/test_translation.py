        ...
        with patch('celery.result.AsyncResult') as mock_async_result:
            ...
            mock_task.result = {
                ...
                'translated_data': {
                    ...
                    'projects': [{'id': 1, 'title': 'Test Project', 'description': 'Translated description'}]
                },
...
class TranslationTaskTests(TestCase):
    ...
    def test_translate_cv_content_success(self, mock_openai_class):
        ...
        bio_response.choices[0].message.content = "Ingeniero de software con experiencia en Python y desarrollo web."

        # Mock project description translation
        project_response = MagicMock()
        project_response.choices = [MagicMock()]
        project_response.choices[0].message.content = "Desarrolló una aplicación web moderna usando TECH_TERM_0 y TECH_TERM_1."
        ...
        mock_client.chat.completions.create.side_effect = [
            bio_response, project_response, skill_response
        ]
    ...
    def test_translate_cv_content_technical_terms_protection(self, mock_openai_class):
        ...
        bio_response.choices[0].message.content = "Experto en Python, Django, y React desarrollo con gestión de base de datos PostgreSQL."
        ...
        technical_terms_present = any(term in translated_bio for term in ['Python', 'Django', 'React', 'PostgreSQL'])
        self.assertTrue(technical_terms_present, f"No technical terms found in: {translated_bio}")
    ...
    def test_translate_cv_content_skill_filtering(self, mock_openai_class):
        ...
        Skill.objects.create(cv=cv_mixed, name="Python", proficiency="expert")  # Technical - should not translate
        Skill.objects.create(cv=cv_mixed, name="Leadership", proficiency="advanced")  # Non-technical - should translate
        Skill.objects.create(cv=cv_mixed, name="Communication", proficiency="intermediate")  # Non-technical - should translate
        ...
        mock_client.chat.completions.create.side_effect = [
            bio_response, leadership_response, communication_response
        ]
        ...
        leadership_skill = next((s for s in translated_skills if 'Liderazgo' in s['name']), None)
        communication_skill = next((s for s in translated_skills if 'Comunicación' in s['name']), None)
...
class TranslationIntegrationTests(TestCase):
    ...
    def test_all_supported_languages(self):
        ...
        supported_languages = [
            'cornish', 'manx', 'breton', 'inuktitut', 'kalaallisut', 'romani',
            'occitan', 'ladino', 'northern_sami', 'upper_sorbian', 'kashubian',
            'zazaki', 'chuvash', 'livonian', 'tsakonian', 'saramaccan', 'bislama'
        ]
